...
export const createObra = async (req: Request, res: Response) => {
  try {
    ...
    let imageUrl = '';
    if (req.file) {
      // Guarda la ruta relativa para frontend, ej: "uploads/obras/image-123456.jpg"
      imageUrl = path.join("uploads/obras", req.file.filename);
    }
    ...
    const insertObraQuery = `
      INSERT INTO Obra (
        ...
        obr_alto_cm, obr_ancho_cm, obr_profundidad_cm, obr_diametro_cm,
        obr_descripcion_formal, obr_detalles_firma, obr_observaciones,
        obr_url_foto, obr_estado_condicion, obr_estado_integridad,
        obr_procedencia, obr_cultura_tradicion, obr_epoca_estilo,
        obr_valor_avaluo, obr_moneda_avaluo, obr_responsable_avaluo,
        obr_fecha_avaluo, obr_propietario_original, obr_documentos_relacionados,
        obr_bibliografia, obr_fecha_ingreso, obr_fuente_adquisicion,
        obr_metodo_adquisicion, obr_entidad_responsable, obr_cla_fk, obr_art_fk, obr_lu_fk
      ) VALUES (
    ...
    const obraValues = [
      obraData.obr_mcf, obraData.obr_numeros_anteriores, obraData.obr_titulo,
      obraData.obr_fecha_realizacion, obraData.obr_alto_cm, obraData.obr_ancho_cm,
      obraData.obr_profundidad_cm, obraData.obr_diametro_cm, obraData.obr_descripcion_formal,
      obraData.obr_detalles_firma, obraData.obr_observaciones, imageUrl,
      obraData.obr_estado_condicion, obraData.obr_estado_integridad,
      obraData.obr_procedencia, obraData.obr_cultura_tradicion, obraData.obr_epoca_estilo,
      obraData.obr_valor_avaluo, obraData.obr_moneda_avaluo, obraData.obr_responsable_avaluo,
      obraData.obr_fecha_avaluo, obraData.obr_propietario_original,
      obraData.obr_documentos_relacionados, obraData.obr_bibliografia,
      obraData.obr_fecha_ingreso, obraData.obr_fuente_adquisicion,
      obraData.obr_metodo_adquisicion, obraData.obr_entidad_responsable,
      obraData.obr_cla_fk, obraData.obr_art_fk, obraData.obr_lu_fk
    ];

    const result = await client.query(insertObraQuery, obraValues);
    const newObraId = result.rows[0].obr_id;

    // Manejo de material y técnica (igual que antes)
    if (material_nombre) {
      let matRes = await client.query('SELECT mat_id FROM Material WHERE mat_nombre=$1', [material_nombre]);
      let matId = matRes.rows.length ? matRes.rows[0].mat_id :
                  (await client.query('INSERT INTO Material(mat_nombre) VALUES($1) RETURNING mat_id', [material_nombre])).rows[0].mat_id;
      await client.query('INSERT INTO Obra_material(obr_id_fk, mat_id_fk) VALUES($1,$2)', [newObraId, matId]);
    }

    if (tecnica_nombre) {
      let tecRes = await client.query('SELECT tec_id FROM Tecnica WHERE tec_nombre=$1', [tecnica_nombre]);
      let tecId = tecRes.rows.length ? tecRes.rows[0].tec_id :
                  (await client.query('INSERT INTO Tecnica(tec_nombre) VALUES($1) RETURNING tec_id', [tecnica_nombre])).rows[0].tec_id;
      await client.query('INSERT INTO Obra_tecnica(obr_tec_obr_fk, obr_tec_tec_fk) VALUES($1,$2)', [newObraId, tecId]);
    }

    await client.query('COMMIT');
    res.status(201).json({ message: 'Obra creada', obra_id: newObraId, imageUrl });
  } catch (err) {
    console.error(err);
